...
- (Boolean)updatePropsForPicker:(UIDatePicker *)picker props:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps {
    ...
    if (oldPickerProps.timeZoneName != newPickerProps.timeZoneName) {
        if (newPickerProps.timeZoneName != NULL && [[NSTimeZone knownTimeZoneNames] containsObject:newPickerProps.timeZoneName]) {
            [shadowPickerView.picker setTimeZone:[[NSTimeZone alloc] initWithName:newPickerProps.timeZoneName]];
        } else {
